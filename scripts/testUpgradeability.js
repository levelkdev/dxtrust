...
/*
-- Quick upgradeability test ro be run in kovan or mainnet
*/
...
const DATContractUpgrade = Contracts.getFromLocal('DecentralizedAutonomousTrustUpgrade');
...
const deployOptions = {
  ...
  control: '0xC4B60a931929d3ed0AC423F9Ea80e5962726dA73',
...
async function main() {
  ...
  contracts.datImplementation = await DATContractUpgrade.new({
    from: deployer
  });
  console.log('Contract upgrade tx:',
    await contracts.proxyAdmin.methods.upgrade(contracts.datProxy.address, contracts.datImplementation.address)
      .send({ from: deployer })
  );
  contracts.dat = DATContractUpgrade.at(contracts.datProxy.address);
  console.log(`DAT proxy upgraded to implementation ${contracts.datImplementation.address}`);

  // console.log('DAT contract upgraded returns 0:', await contracts.dat.methods.newNumber().call());  
  await contracts.dat.methods.saveNumber(666).send({ from: deployer });
  console.log('DAT contract upgraded returns 666:', await contracts.dat.methods.newNumber().call());
